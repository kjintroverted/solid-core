...
function newThing(name) {
  let id = "".concat(name, "-").concat((0, _nanoid.nanoid)());
...
function setAllAttr(thing, struct, data) {
...
async function saveThing(thing) {
  ...
  await (0, _solidClient.saveSolidDatasetAt)(dataURL, dataset, {
    fetch: _solidClientAuthnBrowser.fetch
  });
  return isTemp(thing.url) ? appDataSetURL + "#" + getThingNameFromTempURL(thing.url) : thing.url;
}
